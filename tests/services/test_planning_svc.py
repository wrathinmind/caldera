...
def setup_planning_test(loop, ability, agent, operation, data_svc, init_base_world):
    ...
    sability = ability(ability_id='124', executor='sh', platform='darwin', test=BaseWorld.encode_string('whoami'),
                       singleton=True, variations=[])
    ...
    loop.run_until_complete(data_svc.store(sability))
    ...
    yield tability, tagent, toperation, sability
